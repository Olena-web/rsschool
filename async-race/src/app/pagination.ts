...
export const numPages = async () => {
  ...
  if (a) {
    const carsNumber = a.length;
    const numPages = Math.ceil(carsNumber / carOnPage);
    if (pageNumber) pageNumber.innerHTML = '';
    for (let i = 1; i < numPages + 1; i += 1) {
      if (pageNumber) {
        pageNumber.innerHTML += '<h4  id = "number-current-page' + `${i}` + '"> Page #' + `${i}` + '</h4>';
      }
    }
    const changePage = function (page: number) {
      ...
      for (let i = (page - 1) * carOnPage; i < page * carOnPage && i < carsNumber; i++) {
        const id = i;
        const name = a[i].name;
        const color = a[i].color;
        if (road)
          road.innerHTML += `
    ...
    if (prevButton)
      prevButton.addEventListener('click', () => {
        prevPage(), checkButtonOpacity();
      });
    if (nextButton)
      nextButton.addEventListener('click', () => {
        nextPage(), checkButtonOpacity();
      });
  }
};
void numPages();
